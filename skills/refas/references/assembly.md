...
Physical-part members must retain exactly one P01 `AGGREGATES_INTO` rigid-link membership. The scoped P01 mechanism projection binds that aggregation relation, the member's effective rigid-link identity, and the member's **resolved pose relative to the mechanism frame**. Rigid-link members use themselves as their effective rigid link. Every mechanism member must resolve onto a rigid link incident to at least one of that mechanism's realized P04 joints, and every realized joint must have at least one physically incident mechanism member. A part on an unrelated body cannot become mechanism structure merely because it shares a frame root or a semantic name.
