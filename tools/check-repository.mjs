...
async function main() {
  ...
  assert.equal(packageJson.version,'1.0.2','package version must be 1.0.2');
  const canonical = await fs.readFile(path.join(ROOT,'skills/refas/scripts/lib/canonical.mjs'),'utf8');
  assert.match(canonical,/REFAS_VERSION = '1\.0\.2'/u,'runtime version must match package version');
