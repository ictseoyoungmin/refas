...
export const REFERENCE_INTERFACE_KINDS = Object.freeze(['joint-gap', 'joint-boundary', 'seam', 'overlap-boundary', 'necked-transition', 'contact-boundary', 'unknown']);
...
function normalizeVisibility(value, label) {
...
function normalizePrimitiveBase(raw, label) {
  return {id: assertId(raw?.id, `${label}.id`), importance: normalizeImportance(raw?.importance, `${label}.importance`), evidenceRefs: normalizeStrings(raw?.evidenceRefs, `${label}.evidenceRefs`, {required: true})};
}
...
export function createReferenceGeometry(input = {}) {
  const {
    scopeId, sourceSha256, anchors = [], chains = [], axes = [], contacts = [], occlusions = [], negativeSpaces = [], contours = [], dimensions = [], attestation,
  } = input;
  const segmentationDeclared = Object.prototype.hasOwnProperty.call(input, 'segments') || Object.prototype.hasOwnProperty.call(input, 'interfaces');
  ...
  const normalizedAnchors = anchors.map((raw, index) => {
    ...
    return {...base, xy: normalizePoint(raw?.xy, `anchors[${index}].xy`), visibility: normalizeVisibility(raw?.visibility, `anchors[${index}].visibility`), confidence, semanticRole: String(raw?.semanticRole ?? '')};
  });
  ...
  const normalizedChains = chains.map((raw, index) => {
    ...
    const anchorIdsValue = (raw?.anchorIds ?? []).map((value, i) => assertId(value, `chains[${index}].anchorIds[${i}]`));
  ...
  const normalizedAxes = axes.map((raw, index) => {
    ...
    const fromAnchorId = assertId(raw?.fromAnchorId, `axes[${index}].fromAnchorId`), toAnchorId = assertId(raw?.toAnchorId, `axes[${index}].toAnchorId`);
  ...
  const normalizedSegments = segments.map((raw, index) => {
    ...
    const anchorIdsValue = (raw?.anchorIds ?? []).map((value, i) => assertId(value, `segments[${index}].anchorIds[${i}]`));
    for (const id of anchorIdsValue) if (!anchorIds.has(id)) throw new Error(`segments[${index}] references unknown anchor: ${id}`);
    return {...base, label: String(raw?.label ?? ''), polygon: normalizePolyline(raw?.polygon, `segments[${index}].polygon`, {minimum: 3, closed: true}), anchorIds: anchorIdsValue, visibility: normalizeVisibility(raw?.visibility, `segments[${index}].visibility`), separation};
  });
  ...
  const normalizedInterfaces = interfaces.map((raw, index) => {
    ...
    const subjectSegmentId = assertId(raw?.subjectSegmentId, `interfaces[${index}].subjectSegmentId`), objectSegmentId = assertId(raw?.objectSegmentId, `interfaces[${index}].objectSegmentId`);
    ...
    const kind = String(raw?.kind ?? 'unknown').toLowerCase(), separation = String(raw?.separation ?? 'uncertain').toLowerCase();
    ...
    return {...base, subjectSegmentId, objectSegmentId, kind, separation, boundary: normalizePolyline(raw?.boundary, `interfaces[${index}].boundary`), visibility: normalizeVisibility(raw?.visibility, `interfaces[${index}].visibility`)};
  });

  const normalizedContacts = contacts.map((raw, index) => {
    const base = normalizePrimitiveBase(raw, `contacts[${index}]`);
    const aAnchorId = assertId(raw?.aAnchorId, `contacts[${index}].aAnchorId`), bAnchorId = assertId(raw?.bAnchorId, `contacts[${index}].bAnchorId`);
  ...
  const knownGeometryIds = new Set([...normalizedAnchors, ...normalizedChains, ...normalizedAxes, ...normalizedSegments].map((item) => item.id));
  const normalizedOcclusions = occlusions.map((raw, index) => {
    const base = normalizePrimitiveBase(raw, `occlusions[${index}]`);
    const frontId = assertId(raw?.frontId, `occlusions[${index}].frontId`), backId = assertId(raw?.backId, `occlusions[${index}].backId`);
  ...
  const normalizedNegativeSpaces = negativeSpaces.map((raw, index) => ({...normalizePrimitiveBase(raw, `negativeSpaces[${index}]`), polygon: normalizePolyline(raw?.polygon, `negativeSpaces[${index}].polygon`, {minimum: 3, closed: true})}));
  const normalizedContours = contours.map((raw, index) => ({...normalizePrimitiveBase(raw, `contours[${index}]`), points: normalizePolyline(raw?.points, `contours[${index}].points`, {minimum: raw?.closed === true ? 3 : 2}), closed: raw?.closed === true}));
  const normalizedDimensions = dimensions.map((raw, index) => {
    const base = normalizePrimitiveBase(raw, `dimensions[${index}]`);
    const aAnchorId = assertId(raw?.aAnchorId, `dimensions[${index}].aAnchorId`), bAnchorId = assertId(raw?.bAnchorId, `dimensions[${index}].bAnchorId`);
  ...
  uniqueIds([...normalizedAnchors, ...normalizedChains, ...normalizedAxes, ...normalizedSegments, ...normalizedInterfaces, ...normalizedContacts, ...normalizedOcclusions, ...normalizedNegativeSpaces, ...normalizedContours, ...normalizedDimensions]);
  ...
  if (segmentationDeclared) Object.assign(policy, {observedSegmentationPrecedesPhysicalAssembly: true, segmentSeparationMayRemainUncertain: true});
  const payload = {
    ...
    axes: normalizedAxes,
    ...(segmentationDeclared ? {segments: normalizedSegments, interfaces: normalizedInterfaces} : {}),
...
export function validateReferenceGeometry(geometry) {
  ...
  try {
    const segmentationDeclared = Object.prototype.hasOwnProperty.call(geometry ?? {}, 'segments') || Object.prototype.hasOwnProperty.call(geometry ?? {}, 'interfaces');
    const recreated = createReferenceGeometry({
      scopeId: geometry?.scopeId, sourceSha256: geometry?.sourceSha256, anchors: geometry?.anchors, chains: geometry?.chains, axes: geometry?.axes,
      ...(segmentationDeclared ? {segments: geometry?.segments ?? [], interfaces: geometry?.interfaces ?? []} : {}),
      contacts: geometry?.contacts, occlusions: geometry?.occlusions, negativeSpaces: geometry?.negativeSpaces, contours: geometry?.contours, dimensions: geometry?.dimensions,
    ...
    const payload = structuredClone(geometry); delete payload.geometryDigest;
    ...
    if (segmentationDeclared && (policy.observedSegmentationPrecedesPhysicalAssembly !== true || policy.segmentSeparationMayRemainUncertain !== true)) errors.push('observed segmentation policy is missing');
