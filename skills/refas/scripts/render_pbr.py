...
def pbr_shade(base, normal, view, metallic, roughness):
    ...
    lights = [([-0.55, 0.75, 0.65], 3.4, [1.0, 0.96, 0.90]), ([0.72, 0.18, 0.54], 1.5, [0.70, 0.82, 1.0]), ([-0.15, -0.62, 0.77], 0.65, [1.0, 0.76, 0.58])]
...
def render(primitives, position, target, output, *, size, mode, up_hint, deadline):
    right, up, forward = camera_basis(position, target, up_hint); position = np.asarray(position, dtype=np.float64)
    scale = math.tan(math.radians(31) / 2); background = np.array([15, 18, 23], dtype=np.uint8)
    ...
    for primitive in primitives:
        ...
        for tri in primitive.indices:
            ...
            else: color=pbr_shade(primitive.color,normals,views,primitive.metallic,primitive.roughness)
...
def main():
    ...
    try:
        for name,direction,up,mode,label in specs:
            pos=center+normalize(local_to_world(direction,basis))*distance; target=center; path=staging/f'{name}.png'; rec=render(primitives,pos,target,path,size=a.size,mode=mode,up_hint=local_to_world(up,basis),deadline=deadline); frames.append({**rec,'viewId':name,'label':label,'absolutePath':str(path)})
        board=staging/'pbr-review-board.png'; make_board(Path(a.reference).resolve() if a.reference else None,frames,board)
        rig={"lights":"three-directional-fixed-v1","background":[15,18,23],"cameraFovY":31}; color={"exposure":0,"toneMapping":"Reinhard","outputColorSpace":"sRGB"}
