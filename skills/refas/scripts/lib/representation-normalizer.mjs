...
const NORMALIZER_KEYS = new Set(['id', 'backend', 'version', 'implementationDigest']);
...
const RAW_NORMALIZER_KEYS = new Set(['id', 'backend', 'version', 'implementationDigest', 'normalize']);
...
const CANONICAL_POLICY = Object.freeze({
  ...
  persistedReadingsMustReplayFromVerifiedBytes: true,
  normalizerImplementationIsDigestBound: true,
...
const SEMANTIC_JSON_NORMALIZER_IMPLEMENTATION_DIGEST = digestJson({
  schema: 'refas.normalizer-implementation-registration/v1',
  id: 'refas-semantic-json-normalizer',
  backend: 'refas-semantic-json',
  version: '1',
  implementationId: 'builtin:representation-normalizer:semantic-json:v1',
});
...
function normalizerDescriptor(raw, label='normalizer') {
  return {
    id: assertId(raw.id, `${label}.id`),
    backend: text(raw.backend, `${label}.backend`, {maxLength:256}),
    version: text(raw.version, `${label}.version`, {maxLength:128}),
    implementationDigest: assertDigest(raw.implementationDigest, `${label}.implementationDigest`),
  };
}
function normalizeNormalizer(raw) {
  ...
  return {...normalizerDescriptor(raw), normalize:raw.normalize};
}
function persistedNormalizer(raw){
  assertKnownKeys(raw,NORMALIZER_KEYS,'normalizer');
  return normalizerDescriptor(raw);
}
...
function normalizerInput(manifest,capacityProfile,artifacts){return Object.freeze({manifest:deepFreeze(cloneJson(manifest,'manifest')),obligations:deepFreeze(cloneJson(capacityProfile.obligations,'obligations')),artifacts:Object.freeze(artifacts.map((a)=>Object.freeze({...a})))});}
async function executeNormalizer(n,capacityProfile,manifest,artifacts){const raw=await n.normalize(normalizerInput(manifest,capacityProfile,artifacts));assertKnownKeys(raw,RAW_RESULT_KEYS,'normalizer result');if(!Array.isArray(raw.readings))throw new Error('normalizer result.readings must be an array');return raw.readings.map(normalizeRawReading).sort((a,b)=>a.obligationId.localeCompare(b.obligationId));}
export async function runRepresentationNormalizer({normalizationId,normalizer,capacityProfile,manifest,files}={}){const n=normalizeNormalizer(normalizer);assertProfileManifestBinding(capacityProfile,manifest);if(n.backend!==manifest.adapter.backend)throw new Error(`normalizer backend ${n.backend} does not match P12 export backend ${manifest.adapter.backend}`);const artifacts=verifiedArtifacts(manifest,files);const readings=await executeNormalizer(n,capacityProfile,manifest,artifacts),entries=buildEntries(capacityProfile,manifest,readings),semanticDigest=digestJson(semanticProjection(entries));const payload={schema:NORMALIZED_REPRESENTATION_SCHEMA,normalizationId:assertId(normalizationId,'normalizationId'),normalizer:normalizerDescriptor(n),sourceBinding:sourceBinding(manifest,capacityProfile),entries,semanticDigest,policy:{...CANONICAL_POLICY}};return deepFreeze({...payload,normalizationDigest:digestJson(payload)});}
