<div align="center">
  ...
  <h1>RefAs</h1>
  <p>
    <img src="https://img.shields.io/badge/version-v1.0.4-6f5a46" alt="Version v1.0.4">
...
Open [`demo/index.html`](demo/index.html) for a dependency-free overview of the current 1.0.4 capability boundary and the repository's reproducible examples.
...
RefAs does **not** automatically establish unseen manufacturer-internal mechanisms, calibrated mass/inertia/collider/actuator truth, real-world scale, unknown material composition, or an unambiguous full 3D terminal orientation from a single ambiguous view. Version 1.0.4 may construct hidden form as explicit `inferred` or `engineered` state when its typed basis justifies that construction; this is not a claim that the photographed manufacturer used the same hidden mechanism. See [Known limitations](docs/known-limitations.md).
