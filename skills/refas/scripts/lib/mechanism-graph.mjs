...
function effectiveMemberRigidLinkId(identityGraph,entity,memberId){if(entity.kind==='rigid-link')return entity.id;const matches=identityGraph.relations.filter((relation)=>relation.kind==='AGGREGATES_INTO'&&relation.sourceId===entity.id);if(matches.length!==1)throw new Error(`mechanism physical-part member ${memberId} requires exactly one AGGREGATES_INTO relation`);return matches[0].targetIds[0];}
...
function mechanismIncidence(identityGraph,mechanisms,jointById){
  ...
  return mechanisms.map((mechanism)=>{
    ...
    const members=mechanism.members.map((member)=>{const entity=entityById.get(member.physicalIdentityId);if(!entity)throw new Error(`mechanism member ${member.id} references unknown physical identity: ${member.physicalIdentityId}`);const effectiveRigidLinkId=effectiveMemberRigidLinkId(identityGraph,entity,member.id);if(!incidentLinkSet.has(effectiveRigidLinkId))throw new Error(`mechanism member ${member.id} effective rigid link ${effectiveRigidLinkId} is not incident to any realized articulation joint`);const incidentJointIds=realizedJoints.filter((joint)=>joint.incidentLinkIds.includes(effectiveRigidLinkId)).map((joint)=>joint.virtualJointId).sort();return{memberId:member.id,effectiveRigidLinkId,incidentJointIds};}).sort((a,b)=>a.memberId.localeCompare(b.memberId));
