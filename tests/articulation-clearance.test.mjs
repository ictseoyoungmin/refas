...
const supportBinding = (childId, parentId, proofAttachmentId) => ({childId, parentId, proofAttachmentId, proofChildModuleId: childId, proofParentModuleId: parentId, evidenceRefs: [`reviews/${proofAttachmentId}.json`]});
const gapBound = () => ({counterpartId: 'housing', proofAttachmentId: 'panel-housing-gap', subjectModuleId: 'panel', counterpartModuleId: 'housing', minimumClearance: .08, maximumClearance: .12, evidenceRefs: ['reviews/panel-housing-gap.json']});
...
function clearanceSemantics() {
  return createAttachmentSemantics({
    ...
    relations: [R('housing-free', 'FREE', 'housing'), R('bracket-follow', 'RIGID_FOLLOW', 'bracket', ['housing']), R('panel-clearance', 'SUPPORTED_CLEARANCE', 'panel', ['bracket'])],
  });
}

function clearanceContract(attachmentSemantics) {
  return createSupportedClearance({
    attachmentSemantics, id: 'panel-gap-contract', relationId: 'panel-clearance', supportPathEntityIds: ['panel', 'bracket', 'housing'],
    supportProofBindings: [supportBinding('panel', 'bracket', 'panel-bracket-support'), supportBinding('bracket', 'housing', 'bracket-housing-support')],
    clearanceBounds: [gapBound()], evidenceRefs: ['source/panel.png'],
  });
}

function realizedProof({gap = .1} = {}) {
  const payload = {
    schema: 'refas.realized-assembly-proof/v1', valid: true, errors: [], moduleChecks: [],
    attachmentChecks: [
      ...
      {id: 'panel-housing-gap', childModuleId: 'panel', parentModuleId: 'housing', pass: true, supportDerivedFromContact: true, penetrationDepth: 0, signedClearance: gap},
    ],
...
test('bounded revolute articulation rotates around the declared joint pivot without changing geometry', () => {
  ...
  const joint = createArticulatedJoint({attachmentSemantics, id: 'door-revolute', relationId: 'door-hinge', ownerJointFrame: I(), subjectJointFrame: I([1, 0, 0]), minimumAngle: -Math.PI / 2, maximumAngle: Math.PI / 2, evidenceRefs: ['model/door-hinge.json']});
...
test('supported clearance requires a semantic support chain and exact realized module-pair evidence', () => {
  const attachmentSemantics = clearanceSemantics(), contract = clearanceContract(attachmentSemantics), proof = realizedProof();
  ...
  assert.equal(report.supportResults.every((result) => result.pass && result.modulePairPass), true);
  assert.equal(report.clearanceResults[0].signedClearance, .1);
  assert.equal(report.clearanceResults[0].modulePairPass, true);
...
test('supported clearance blocks an out-of-range gap instead of inventing direct contact', () => {
  const attachmentSemantics = clearanceSemantics(), contract = clearanceContract(attachmentSemantics);
...
test('a passing proof from the wrong realized module pair cannot satisfy a semantic support edge', () => {
  const attachmentSemantics = clearanceSemantics(), contract = clearanceContract(attachmentSemantics);
  const proof = realizedProof();
  const payload = structuredClone(proof); delete payload.proofDigest;
  payload.attachmentChecks.find((check) => check.id === 'panel-bracket-support').childModuleId = 'housing';
  const mismatchedProof = {...payload, proofDigest: digestJson(payload)};
  const report = evaluateSupportedClearance({contract, attachmentSemantics, realizedAssemblyProof: mismatchedProof});
  assert.equal(report.status, 'BLOCKED');
  assert.equal(report.supportResults.find((result) => result.proofAttachmentId === 'panel-bracket-support').modulePairPass, false);
});
...
test('support path cannot invent an undeclared semantic edge', () => {
  ...
  assert.throws(() => createSupportedClearance({
    ...
    supportProofBindings: [supportBinding('panel', 'housing', 'fake-support')], clearanceBounds: [gapBound()], evidenceRefs: ['source/panel.png'],
...
test('articulation and clearance artifacts remain tamper detectable', () => {
  ...
  const tamperedJoint = structuredClone(joint); tamperedJoint.limits.maximumAngle = 2;
  assert.equal(validateArticulatedJoint(tamperedJoint, articulatedSemantics).valid, false);

  const attachmentSemantics = clearanceSemantics(), contract = clearanceContract(attachmentSemantics);
  const tampered = structuredClone(contract); tampered.clearanceBounds[0].maximumClearance = 9;
