...
export async function inspectCertificationProjectionEvidence(root, state, head, visualReview) {
  const errors = [];
  const geometryArtifacts = head.artifactRefs.filter((artifact) => artifact.kind === 'reference-geometry');
  const realizedArtifacts = head.artifactRefs.filter((artifact) => artifact.kind === 'realized-projection');
  ...
  const geometryArtifact = geometryArtifacts.length === 1 ? geometryArtifacts[0] : null;
  const realizedArtifact = realizedArtifacts.length === 1 ? realizedArtifacts[0] : null;
  let proof = null;
  let geometry = null;

  if (geometryArtifact && realizedArtifact) {
    try {
      const geometryPath = await readBoundArtifact(root, geometryArtifact, 'reference-geometry');
      const realizedPath = await readBoundArtifact(root, realizedArtifact, 'realized-projection');
      geometry = await readJson(geometryPath);
      proof = await readJson(realizedPath);
      const geometryValidation = validateReferenceGeometry(geometry);
      if (!geometryValidation.valid) errors.push(`reference geometry is invalid: ${geometryValidation.errors.join('; ')}`);
      const realizedValidation = validateRealizedProjection(proof);
      ...
      if (glbArtifacts.length !== 1) {
        errors.push('source-bound certification requires exactly one GLB artifact matching the realized projection asset digest');
      } else {
        const glbPath = await readBoundArtifact(root, glbArtifacts[0], 'realized GLB');
        const glb = await fs.readFile(glbPath);
        const verification = verifyRealizedProjection({proof, referenceGeometry: geometry, glb});
      ...
      const blocking = findingsFromProjectionFit(proof.projectionFit).filter((finding) => finding.blocking === true);
      ...
      errors.push(`realized projection evidence unavailable: ${error.message}`);
    }
  }
