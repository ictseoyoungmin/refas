...
export const HOST_SESSION_STATUSES = Object.freeze([
  'idle','active','paused','cancelling','cancelled','blocked','reviewable','completed','failed',
]);
...
function deriveHostStatus(state, stored) {
  ...
  switch (state.status) {
    ...
    case 'closure-eligible': return 'reviewable';
...
async function verifiedGlbArtifact(root, checkpoint) {
  ...
  return deepFreeze({schema:'refas.content-reference/v1',kind:'glb',path:rawPath.split(path.sep).join('/'),sha256:artifact.sha256,sizeBytes:artifact.sizeBytes});
}
...
export async function loadHostSession(root, {sessionId = null, projectId = null} = {}) {
  root = projectRoot(root);
  const stored = await readHostState(root);
