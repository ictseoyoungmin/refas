...
def make_board(title, ancestry, images, metrics, output):
    ...
    draw.text((14, 40), f"ANCESTRY: {' > '.join(ancestry)}   IoU {metrics['silhouetteIoU']:.4f} (aid only)", fill=(164, 178, 196), font=font(14))
...
def main():
    ...
    for scope_id in spec["scopeIds"]:
        ...
        split = src_crop.copy(); split.paste(render_crop.crop((render_crop.width // 2, 0, render_crop.width, render_crop.height)), (render_crop.width // 2, 0))
        split_draw = ImageDraw.Draw(split); split_draw.line((render_crop.width // 2, 0, render_crop.width // 2, render_crop.height), fill=(255, 210, 64), width=3)
        ...
        edges[source_edges] = [255, 69, 78]; edges[render_edges] = np.maximum(edges[render_edges], [42, 205, 255])
        edge_image = Image.fromarray(edges)
        source_mask, render_mask = foreground_mask(src_crop), foreground_mask(render_crop)
        union = np.logical_or(source_mask, render_mask); intersection = np.logical_and(source_mask, render_mask)
        ...
        context = render.copy(); context_draw = ImageDraw.Draw(context); context_draw.rectangle(box, outline=(255, 210, 64), width=max(2, render.width // 180))
        ...
        for landmark in spec.get("landmarks", []):
            ...
            local_landmarks.append({"id": landmark["id"], "sourceNormalized": landmark["source"], "mappedRenderNormalized": mapped_point,
                                    "declaredRenderNormalized": landmark["render"], "residualNormalized": residual,
        ...
        for dimension in spec.get("dimensions", []):
            if dimension["a"] in by_id and dimension["b"] in by_id:
                ...
                dimensions.append({"id": dimension["id"], "sourceNormalizedLength": sd, "renderNormalizedLength": rd, "ratioRenderToSource": rd / max(sd, 1e-12), "evidenceClass": "derived-observation-aid"})
        metrics = {"silhouetteIoU": iou, "sourceForegroundPixels": int(source_mask.sum()), "renderForegroundPixels": int(render_mask.sum()),
                   "landmarkResidualRmse": float(np.sqrt(np.mean([x["residualNormalized"] ** 2 for x in local_landmarks]))) if local_landmarks else None}
        scope_dir = out / scope_id; scope_dir.mkdir(exist_ok=True)
        ...
        for label, image in image_set:
            ...
            image_records.append({"label": label, "path": str(path.relative_to(out)), "sha256": sha256(path), "width": image.width, "height": image.height, "evidenceClass": "derived-observation-aid"})
        ...
        image_records.append({"label": "COMPARISON BOARD", "path": str(board_path.relative_to(out)), "sha256": sha256(board_path), "width": 1080, "height": 750, "evidenceClass": "derived-observation-aid"})
        records.extend(image_records)
        scope_reports.append({"scopeId": scope_id, "level": node["level"], "ancestry": ancestry, "sourceRoi": node["roi"], "registeredRenderRoi": child_roi,
                              "metrics": metrics, "landmarks": local_landmarks, "dimensions": dimensions, "images": image_records})

    report = {"schema": SCHEMA, "claimScope": "critique-evidence-only", "source": {"manifestPath": str(source_manifest_path), "manifestSha256": sha256(source_manifest_path), "sha256": source_manifest["sha256"]},
              "render": {"reportPath": str(render_report_path), "reportSha256": sha256(render_report_path), "assetSha256": render_report["asset"]["sha256"], "frameId": spec["frameId"], "frameSha256": frame["sha256"]},
              "registration": {"path": str(registration_path), "fileSha256": sha256(registration_path), "digest": registration["registrationDigest"], "model": registration["model"], "metrics": registration["metrics"]},
              "hierarchy": {"path": str(hierarchy_path), "fileSha256": sha256(hierarchy_path), "digest": hierarchy["hierarchyDigest"]}, "scopes": scope_reports,
    ...
    print(json.dumps({"status": "PASS", "report": str(report_path), "comparisonDigest": report["comparisonDigest"], "scopes": len(scope_reports)}, indent=2))
