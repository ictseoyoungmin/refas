...
export async function analyzeDiscoveryClosure({
  ...
  try {
    targetAuthority = await loadTargetAuthority(skillRoot);
  } catch (error) {
    missing.push(`target-authority:${error.message}`);
  }
  const PUBLIC_API = targetAuthority?.PUBLIC_API ?? {};
  const CAPABILITY_ORDER = targetAuthority?.CAPABILITY_ORDER ?? [];
  const semantic = targetAuthority
    ? await targetAuthority.analyzeSemanticInstructionGraph({skillRoot})
    : {errors: ['target semantic instruction verifier unavailable']};
  for (const error of semantic.errors ?? []) missing.push(`semantic-graph:${error}`);
  const interfaces = targetAuthority
    ? await targetAuthority.analyzeCapabilityInterfaces({skillRoot, exerciseContracts: false})
    : {errors: ['target capability interface verifier unavailable'], executableInterfaces: 0};
  ...
  const templateSchemaOccurrences = [];
  const templateMetadata = new Map();
  for (const template of actualTemplates) {
    try {
      const json = JSON.parse(await fs.readFile(path.join(skillRoot, template), 'utf8'));
      const occurrences = schemaOccurrencesInValue(json).map((item) => ({...item, template}));
      for (const occurrence of occurrences) {
        templateSchemaIds.add(occurrence.schemaId);
        templateSchemaOccurrences.push(occurrence);
      }
      templateMetadata.set(template, {
        rootSchema: typeof json.schema === 'string' ? json.schema : null,
        catalogLike: Boolean(
          json
          && typeof json === 'object'
          && !Array.isArray(json)
          && json.placeholderContract
          && typeof json.placeholderContract === 'object'
          && json.templates
          && typeof json.templates === 'object'
  ...
  for (const schemaId of workerFacingSchemaIds) {
    const authorities = [];
    if (outputSchemaIds.has(schemaId)) authorities.push('graph-output-validation');
    if (schemaCatalog.available && schemaCatalog.ids.has(schemaId)) authorities.push('concrete-schema');
    for (const occurrence of templateSchemaOccurrences.filter((item) => item.schemaId === schemaId)) {
      const metadata = templateMetadata.get(occurrence.template) ?? {};
      if (occurrence.root && schemaId === canonicalTemplateSchemaId(occurrence.template)) {
        authorities.push(`canonical-template-name:${occurrence.template}`);
      } else if (!occurrence.root && occurrence.rootSchema && outputSchemaIds.has(occurrence.rootSchema)) {
        authorities.push(`enclosing-public-validation:${occurrence.rootSchema}`);
      } else if (metadata.catalogLike) {
        authorities.push(occurrence.root
          ? `non-persisted-template-catalog:${occurrence.template}`
          : `non-persisted-catalog-example:${occurrence.template}`);
      }
    }
    const resolved = sorted(authorities);
    schemaAuthorities[schemaId] = resolved;
    if (!resolved.length) missing.push(`schema-unresolved:${schemaId}`);
  }

  let freshWorker = null;
  if (runFreshWorker && !missing.length && !orphan.length && !privateDependencies.length && !unreachable.length) {
    freshWorker = await targetAuthority.runFreshWorkerDogfood({skillRoot});
  ...
  const result = {
    ...
    resolvedWorkerFacingSchemaIds: Object.values(schemaAuthorities).filter((items) => items.length > 0).length,
    schemaAuthorities,
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
 ...
 && typeof value.schema === 'string' ? value.schema : rootSchema;
  if (typeof value.schema === 'string' && /^refas\.[A-Za-z0-9._-]+\/v\d+$/u.test(value.schema)) {
    output.push({
      schemaId: value.schema,
      pointer: `${pointer}.schema`,
...
async function loadTargetAuthority(skillRoot) {
  ...
  if (!Array.isArray(publicApi.CAPABILITY_ORDER)) {
    throw new Error('target public API does not export CAPABILITY_ORDER');
  }
  if (typeof semanticModule.analyzeSemanticInstructionGraph !== 'function') {
    throw new Error('target semantic instruction verifier is unavailable');
  }
  if (typeof interfaceModule.analyzeCapabilityInterfaces !== 'function') {
    throw new Error('target capability interface verifier is unavailable');
  }
  if (typeof freshWorkerModule.runFreshWorkerDogfood !== 'function') {
    throw new Error('target fresh-worker verifier is unavailable');
  }
  return {
    PUBLIC_API: publicApi,
    CAPABILITY_ORDER: publicApi.CAPABILITY_ORDER,
