...
- virtual-joint target: use the selected-joint P04 projection and bind parent/child direction, derived joint frames, reference configuration, and current resolved P01 pose. Also project the current typed-joint fields that define coordinate meaning: selected relation, owner/subject, joint type, axis convention, zero convention, and owner/subject joint frames. The selected attachment relation is checked against current attachment semantics. Limits, evidence, provenance, and the whole attachment-semantics digest are deliberately excluded when those coordinate semantics are unchanged. The current public articulated-joint schema is revolute-only, so its runtime coordinate class is `ROTARY`;
...
An upstream change that alters the selected target's coordinate class, selected joint direction/frame/reference pose, selected typed-joint axis/zero/frame semantics, selected attachment relation, selected actuator routing, selected transmission/articulation dependency, or exact P01 runtime relation invalidates the P09 target binding. Unrelated P04 joints, joint limits/evidence, P07 capability limits, runtime endpoints, or controller tuning must not.
