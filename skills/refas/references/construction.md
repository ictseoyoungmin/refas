...
When a construction backend exposes semantic geometry parameters, use `refas.parameter-fit-plan/v1` and the `fit-parameters` CLI to jointly search coupled proportions, sections, curvature, or thickness. Read `references/parameter-fitting.md` before starting. Every evaluation must produce exact candidate GLB and render content references; analytic-only proxy tests do not replace the actual-render fixture or final source-bound inspection.
